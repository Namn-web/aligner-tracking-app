...
--clay-dark:  rgba(63, 55, 43, 0.10);      /* 右下シャドウ（ウォーム系） */

/* 押し込み（凹）：入力フィールド・溝・アクティブ状態 */
--clay-pressed:
  inset 2px 2px 5px rgba(63, 55, 43, 0.10),
  inset -2px -2px 4px rgba(255, 255, 255, 0.6);
```

| パターン | 使う場面 |
|---|---|
| 深い凸（大きな影） | **装着トグルとボトムナビだけの特権**。他には使わない |
| 浅い凸（`0 2px 10px` 程度＋薄ボーダー） | カード、リスト行、ドット |
...
**全要素に深い影を付けない**こと。全部が浮くと全部が沈んで見える。深さはトグルの特権。
...
### プライマリボタン
```css
/* 白文字とのコントラスト確保のため、ボタン面は明るい--primaryでなく深緑グラデを使う */
background: radial-gradient(circle at 38% 35%, #54C495, #2FA076);
color: #FFFFFF;
border: none;
border-radius: 12px;
padding: 14px;
